...
setInterval( givesCurrentDateTime, 1000);
...
function httpRequestHandler(request, response) {
	...
	try {
		...
		if (parseUrl.path != ""
			...
			else if (request.url == '/execFilesToUpload') {
		try{
            executeCommand("mount -o remount,rw /", null);
            var form = new _formidable.IncomingForm();
            form.parse(request, function (err, fields, files) {
                var oldpath = files.theFile.path;
                var newpath = '/tmp/farecoubr.tar.gz';// plus la date dans le nom
			...
			process.chdir('/tmp');
                        ...
                        var report = processSync(`tar zxvf farecoubr.tar.gz`);
			executeCommand(`rm upload_*`);

                        if (report.stderr != null) {
                            failure(report.stderr);
                            return;l
                        } else {
                           var report = processSync(`./updateubr`);
                            if (report.stderr != null) {
                                failure(report.stderr);
                                 return;
                             } 
